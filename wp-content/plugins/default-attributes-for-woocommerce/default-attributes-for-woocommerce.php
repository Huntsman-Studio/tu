...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || get_site_option( 'active_sitewide_plugins') && array_key_exists( 'woocommerce/woocommerce.php', get_site_option( 'active_sitewide_plugins' ) ) ) {
	...
	class WC_DAW {
		...
		public function init_functions() {
			...
			add_action( 'woocommerce_after_add_attribute_fields', [ $this, 'wc_edit_attribute_default' ] );
			add_action( 'woocommerce_after_edit_attribute_fields', [ $this, 'wc_edit_attribute_default' ] );
			add_action( 'woocommerce_attribute_added', [ $this, 'wc_save_attribute_default' ] );
		...
		/**
		 * Gets all the settings for this plugin for @see woocommerce_admin_fields() function
		...
		public function wc_get_settings() {
			$settings = [
				...
				'max_variations'	=> [
					'name'		=> __( 'Max variation for by-stock default attributes calculation', 'woocommerce' ),
				...
				'first_attribute'	=> [
					...
					'desc_tip'	=> __( 'If not all the calculated default attributes are selectable they all will be unset', 'default-attributes-for-woocommerce' ),
					'type'		=> 'checkbox',
					'default'	=> 'no',
				...
				'remove_text'	=> [
					...
					'desc'		=> __( 'In single product pages remove the select options text if default attribute is set', 'default-attributes-for-woocommerce' ),
					'type'		=> 'checkbox',
					'default'	=> 'no',
		...
		/**
		 * Sets the default attributes based on the settings
		...
		public function wc_product_get_default_attributes( $default_attributes, $product ) {
			if ( ! is_admin() && apply_filters( 'daw_defaults_enabled', is_single( $product->get_id() ) && is_product() ) ) {
				...
				if ( false === $transient ) {
					...
					foreach ( $product->get_attributes() as $attribute_slug => $attribute ) {
						if ( $attribute['variation'] ) {
							...
							if ( isset( $stock_default_attributes[ $attribute_slug ] ) ) {
								...
								if ( $attribute_default && 'none' !== $attribute_default && 'first' !== $attribute_default ) {
					...
					set_transient( 'daw_default_attributes_' . $product->get_id(), apply_filters( 'daw_product_get_default_attributes', $default_attributes, $product ), apply_filters( 'daw_transient_expiration', MINUTE_IN_SECONDS, $product ) );
				} else {
					return $transient;
				}
			}
		...
		/**
		 * Adds option to define a default per attribute
		 */
		...
		/**
		 * Saves the attribute default field
		 * @param mixed $id
		 */
		...
		/**
		 * Erases the attribute default field
		 * @param mixed $id
		 */
		...
		/**
		 * Disables out of stock variations
		...
		/**
		 * Increases the ajax variation threshold so the disable options will work on more variations
		 * @param mixed $grey_out
		 * @param mixed $variation
		 * @return bool
		 */
		...
		/**
		 * Creates custom select attributes button text in archive page
		 * @param mixed $args
		 * @return mixed
		 */
		...
		/**
		 * Sets dynamically the add to cart button text for single pages of variable products
		 * @param mixed $args
		 * @return mixed
		 */
		...
		/**
		 * Removes the select options text when there is default
		 * @param mixed $args
		 * @return mixed
		 */
